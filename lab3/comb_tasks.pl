...
range(End, Res):- range(0, End, [], Res).
range(Start, End, Res):- range(Start, End, [], Res).
range(End, End, CurRes, CurRes):- !.
range(Start, End, CurRes, Res):- NewStart is Start + 1, range(NewStart, End, [Start|CurRes], Res).  


% mix_with_a(+PosComb, +Perm, -Word)

mix_with_a(PosComb, Perm, Word):- mix_with_a(PosComb, Perm, Word, [], 0).
mix_with_a([], [], CurWord, CurWord, _).
mix_with_a([H|T], Perm, Word, CurWord, H):- NewI is H + 1, mix_with_a(T, Perm, Word, [a|CurWord], NewI), !.
mix_with_a(PosComb, [H|T], Word, CurWord, I):- NewI is I + 1, mix_with_a(PosComb, T, Word, [H|CurWord], NewI). 


% print_3a_words(+Alphabet, +K)

print_3a_words(Alphabet, K):-
    range(K, PosList), next_combination(PosList, 3, CurPosComb), NewK is K - 3, next_permutation(Alphabet, NewK, CurPerm),
    mix_with_a(CurPosComb, CurPerm, CurWord), write(CurWord), nl, fail. 